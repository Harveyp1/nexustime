...
<body class="bg-gray-950 text-white font-inter">
  <div class="min-h-screen flex items-center justify-center px-4">
    <div class="w-full max-w-2xl bg-gray-900 p-8 rounded-2xl shadow-xl">
      ...
      <form id="onboardingForm" class="space-y-5">
        ...
        <div>
          <label for="companySize" class="block text-sm mb-1">Company Size</label>
          <select id="companySize" class="w-full p-3 bg-gray-800 border border-gray-700 rounded-xl" required></select>
        </div>
        ...
        <div>
          <label for="plan" class="block text-sm mb-1">Plan</label>
          <select id="plan" class="w-full p-3 bg-gray-800 border border-gray-700 rounded-xl" required></select>
        </div>

        <div>
          <label for="logo" class="block text-sm mb-1">Upload Logo (Optional - PNG 64x64)</label>
          <input type="file" id="logo" accept="image/png" class="w-full bg-gray-800 text-white rounded-xl border border-gray-700 p-2" />
          <p id="logoMsg" class="text-red-500 text-sm mt-1"></p>
        </div>
  ...
  <script src="/scripts/company-options.js"></script>
  <script src="/scripts/validate-logo-upload.js"></script>
  ...
  <script>
    document.getElementById("onboardingForm").addEventListener("submit", async (e) => {
      ...
      let logoUrl = "";
      if (logoFile) {
        const storageRef = storage.ref(`workspace-logos/${user.uid}-${Date.now()}.png`);
